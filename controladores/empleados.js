...
let getEmpleados=(req,res)=>{
    ...
    connection.query(`SELECT * FROM empleados `, (err, result, fields)=>{
        ...
        if(result.length < 1)  res.status(404).send({message:`No se encontraron empleados`});
...
let getempleado=(req,res)=>{
    ...
    connection.query(`SELECT * FROM empleados WHERE idempleado = ${idEmpleado} `, (err, result, fields)=>{
        if(err)  res.status(500).send({message:`Error en la consulta ${err}`});
        if(result.length < 1)  res.status(404).send({message:`No se encontro al empleado`});
        if(!err && result.length >= 1){
            res.status(200).send({result:result});
        }
        connection.destroy();
...
let nuevoEmpleado=(req,res)=>{
    ...
    if(!data.nombres || !data.app_pat || !data.app_mat || !data.callenum || !data.colonia || !data.estado || !data.municipio || !data.poblacion || !data.telefono || !data.negocio || !data.sucursal || !data.usuario || !data.password || !data.status || !data.puesto || !data.derecho_esp) return res.status(403).send({message:`ERROR  no se enviaron todos los datos`});
    var sql = `INSERT INTO empleados VALUES(NULL,'${data.nombres}','${data.app_pat}','${data.app_mat}','${data.callenum}', '${data.colonia}','${data.estado}','${data.municipio}', '${data.poblacion}','${data.telefono}', '${fecha_alta}','${data.negocio}', '${data.sucursal}','${data.usuario}', '${data.password}','${data.status}', '${data.puesto}','${data.derecho_esp}')`;
    var connection = dbConnection();
...
let updateEmpleado = (req,res)=>{
    var idEmpleado = req.params.id;
    var data = req.body;
    if(!data.nombres || !data.app_pat || !data.app_mat || !data.callenum || !data.colonia || !data.estado || !data.municipio || !data.poblacion || !data.telefono || !data.negocio || !data.sucursal || !data.usuario || !data.password || !data.status || !data.puesto || !data.derecho_esp ) return res.status(403).send({message:`ERROR no se enviaron todos los campos`});
...
let login =(req,res)=>{
    ...
    connection.query(sql,(err,result,fields)=>{
        if(err) res.status(500).send({message:`ERROR ${err}`});
        if(result.length >1) res.status(404).send({message:`ERROR contraseña o usuarios incorrectos`});
        if(!err && result.length >= 1 ){
