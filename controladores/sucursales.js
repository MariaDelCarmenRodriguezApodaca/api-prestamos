...
let nuevaSucursal= (req,res)=>{
    var data = req.body;
    if(!data.nombre ||  !data.callenum || !data.colonia || !data.poblacion || !data.municipio || !data.estado ||  !data.status || !data.hora_inicio || !data.hora_fin || !data.telefono || !data.encargado || !data.id_empresa ) return res.status(403).send({message:`Faltaron datos en la peticion`}); 
    var sql=`INSERT INTO sucursales VALUES (null, '${data.nombre}','${data.callenum}','${data.colonia}','${data.poblacion}','${data.municipio}','${data.estado}','${data.status}','${data.hora_inicio}','${data.hora_fin}',${data.telefono},'${data.encargado}','${data.id_empresa}')`;
    var connection = dbConnection();
